<?php
include "conexao.php"; // Inclui a conexão com MySQL + variáveis do Cloudinary ($cloud_name, $api_key, $api_secret)
...
if(isset($_GET['excluir'])) {
    ...
    $res = mysqli_query($conexao, "SELECT imagem_url FROM recado WHERE id = $id");
    ...
    mysqli_query($conexao, "DELETE FROM recado WHERE id = $id") or die("Erro ao excluir: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
$update_sql = "UPDATE recado SET nome='$nome', descricao='$descricao', preco=$preco WHERE id=$id";
mysqli_query($conexao, $update_sql) or die("Erro ao atualizar: " . mysqli_error($conexao));
header("Location: moderar.php");
exit;
}
// Produto a ser editado (via GET)
$editar_id = isset($_GET['editar']) ? intval($_GET['editar']) : 0;
// Seleciona todos os produtos
$produtos = mysqli_query($conexao, "SELECT * FROM recado ORDER BY id DESC");
?>

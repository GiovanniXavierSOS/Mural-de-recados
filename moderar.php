...
if(isset($_POST['atualiza'])){
    ...
    $sql = "UPDATE giovanni2d SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    $id = intval($_GET['id']);
    mysqli_query($conexao, "DELETE FROM giovanni2d WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
$recado_editar = null;
if($editar_id){
    $res = mysqli_query($conexao, "SELECT * FROM giovanni2d WHERE id=$editar_id");
...
<?php
$seleciona = mysqli_query($conexao, "SELECT * FROM giovanni2d ORDER BY id DESC");
